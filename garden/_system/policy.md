...
- Skill 只在用户明确要求 `/learn` 或调用 `skill_manage` 时创建或修改；普通会话不会自动生成 Skill。
...
- 本地 Skills 位于 `~/.hermes/skills/`，默认 Hermes 直接加载；worker 通过 `skills.external_dirs` 共享该目录，profile 本地的同名 Skill 优先。
- `auxiliary.background_review.enabled=false`、`skills.creation_nudge_interval=0`、`skills.write_approval=true`、`skills.ledger=true`、`curator.enabled=false` 固定了显式写入边界。
